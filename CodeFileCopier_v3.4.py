...
"""
==================================================================
 COPIADOR DE CÓDIGO — Python Edition (v3.4)
==================================================================
...
Todas as funcionalidades da versão HTML foram preservadas:
  ...
  • Caminho de entrada editável e recarga de pasta/ZIP preservando seleções
  • Aba Restaurar para reconstrução em pasta ou ZIP a partir do TXT
  • Menus de contexto em campos de texto e listas de arquivos
...
"""
==================================================================
 AJUSTES DE RESPONSIVIDADE (v3.4)
==================================================================
...
# CONSTANTES
# ==================================================================
APP_TITLE = "📁 Copiador de Código — Python Edition v3.4"
...
RESTORE_SEP = '=' * 42
RESTORE_MAX_INPUT_BYTES = 768 * 1024 * 1024
RESTORE_MAX_ENTRIES = 100_000
RESTORE_PREVIEW_LIMIT = 20_000
RESTORE_HEADER_RE = re.compile(
    re.escape(RESTORE_SEP) + r'\n'
    r'Conteúdo de (?P<name>.+?)'
    r'(?: \(caminho: (?P<rel>.+?)\) \[enc: utf-8\])?:\n'
    + re.escape(RESTORE_SEP) + r'\n'
)
RESTORE_FOOTER_MARKERS = (
    '\n' + RESTORE_SEP + '\nEstrutura de pastas:\n',
    '\n' + RESTORE_SEP + '\nArquivos:\n',
)
RESTORE_GHOST_HEADER_RE = re.compile(
    r'\n' + re.escape(RESTORE_SEP) + r'\n'
    r'Conteúdo de .+?(?: \(caminho: .+?\) \[enc: utf-8\])?:\n'
    r'(?:' + re.escape(RESTORE_SEP) + r'\n?)?\n?$',
    re.DOTALL,
)


class OperationCancelled(Exception):
    """Interrupção cooperativa de uma tarefa em segundo plano."""
...
# ==================================================================
# RESTAURAÇÃO DE TXT DO CODEFILECOPIER
# ==================================================================
def _restore_safe_path(raw_path):
    """Normaliza um caminho lógico sem permitir saída da pasta de destino."""
    path = str(raw_path or '').replace('\\', '/')
    if (not path or '\0' in path or path.startswith('/') or
    ...
    if '..' in parts:
        return None
    clean = [part for part in parts if part and part != '.']
    if not clean:
        return None
...
def parse_codefilecopier_txt(txt_path, progress_cb=None, cancel_event=None,
                              max_input_bytes=RESTORE_MAX_INPUT_BYTES,
                              max_entries=RESTORE_MAX_ENTRIES):
    """Interpreta arquivos de projeto e arquivos avulsos gerados pelo app.

    O comportamento acompanha restore_codefilecopier.py e a aba Restaurar
    da versão HTML: aceita blocos com e sem caminho, consolida cabeçalhos
    fantasmas, mantém a última ocorrência de caminhos duplicados e reconhece
    os dois rodapés oficiais.
    """
    progress_cb = progress_cb or (lambda phase, current=0, total=0: None)
    ...
    if size > max_input_bytes:
        raise ValueError(
            f'O TXT ultrapassa o limite de segurança de '
            f'{max_input_bytes // 1024 // 1024} MB.')
    progress_cb('reading', 0, size)
    chunks = []
    bytes_read = 0
    with open(txt_path, 'rb') as fh:
        while True:
            ...
            if bytes_read > max_input_bytes:
                raise ValueError('O TXT ultrapassa o limite de segurança configurado.')
            progress_cb('reading', bytes_read, size)
    raw = b''.join(chunks)
    check_cancel()
    try:
        text = raw.decode('utf-8')
    except UnicodeDecodeError as exc:
        raise ValueError('O arquivo não é um texto UTF-8 válido.') from exc
    text = text.replace('\r\n', '\n')
    progress_cb('parsing', 0, 0)

    footer_positions = [text.find(marker) for marker in RESTORE_FOOTER_MARKERS]
    footer_positions = [position for position in footer_positions if position != -1]
    footer_index = min(footer_positions) if footer_positions else -1
    body = text[:footer_index] if footer_index != -1 else text

    raw_matches = []
    for header_match in RESTORE_HEADER_RE.finditer(body):
        raw_matches.append(header_match)
        if len(raw_matches) > max_entries * 2:
            raise ValueError('Quantidade excessiva de cabeçalhos no TXT.')
        if len(raw_matches) % 1000 == 0:
            check_cancel()
    if not raw_matches:
        raise ValueError(
            'Nenhum cabeçalho de arquivo foi encontrado. '
            'O TXT não está no formato esperado.')

    matches = []
    for index, current in enumerate(raw_matches):
        if index % 500 == 0:
            check_cancel()
            progress_cb('parsing', index, len(raw_matches))
        next_match = raw_matches[index + 1] if index + 1 < len(raw_matches) else None
        current_key = current.group('rel') or current.group('name')
        next_key = (next_match.group('rel') or next_match.group('name')) if next_match else None
        if (next_match is not None and current_key == next_key and
                body[current.end():next_match.start()].strip() == ''):
            continue
        matches.append(current)
    if len(matches) > max_entries:
        raise ValueError(f'O TXT ultrapassa o limite de {max_entries} arquivos.')
    ...
    for index, current in enumerate(matches):
        if index % 250 == 0:
            check_cancel()
            progress_cb('parsing', index, len(matches))
        content_end = matches[index + 1].start() if index + 1 < len(matches) else len(body)
        content = body[current.end():content_end]
        ghost = RESTORE_GHOST_HEADER_RE.search(content)
        logical_path = current.group('rel') or current.group('name')
        if ghost:
            content = content[:ghost.start()]
            warn(f'Fragmento de cabeçalho fantasma removido: {logical_path}')
        elif content.endswith('\n\n'):
            content = content[:-2]
        elif content.endswith('\n'):
            content = content[:-1]

        safe = _restore_safe_path(logical_path)
        if safe is None:
            warn(f'Caminho suspeito ignorado: {logical_path}')
            continue
        normalized, changed = safe
        if changed:
            warn(f'Caminho normalizado: {normalized}')
        if normalized in final_entries:
            warn(f'Caminho duplicado sobrescrito: {normalized}')
    ...
    check_cancel()
    if footer_index == -1:
        warn("Marcador final 'Estrutura de pastas:' ou 'Arquivos:' não encontrado.")
    entries = list(final_entries.values())
    total_chars = sum(entry['chars'] for entry in entries)
    progress_cb('done', len(entries), len(entries))
    return {
        'entries': entries,
        'warnings': warnings,
        'warning_count': warning_count,
        'total_chars': total_chars,
        'footer_found': footer_index != -1,
    }


def restore_entries_to_folder(entries, destination, progress_cb=None, cancel_event=None):
    """Grava entradas validadas em uma pasta, impedindo path traversal."""
    progress_cb = progress_cb or (lambda current, total: None)
    cancel_event = cancel_event or threading.Event()
    ...
    for index, entry in enumerate(entries):
        ...
        try:
            if os.path.commonpath((str(root), str(resolved))) != str(root):
                warnings.append(f"Caminho suspeito ignorado: {entry['path']}")
                continue
        except ValueError:
            warnings.append(f"Caminho suspeito ignorado: {entry['path']}")
            continue
        try:
            ...
            warnings.append(f"Falha ao gravar {entry['path']}: {exc}")
        progress_cb(index + 1, len(entries))
    return restored, warnings


def restore_entries_to_zip(entries, zip_path, progress_cb=None, cancel_event=None):
    """Cria um ZIP offline e substitui o destino apenas após o sucesso."""
    progress_cb = progress_cb or (lambda current, total: None)
    cancel_event = cancel_event or threading.Event()
    ...
    try:
        with zipfile.ZipFile(temp_path, 'w', compression=zipfile.ZIP_DEFLATED,
            ...
            for index, entry in enumerate(entries):
                if cancel_event.is_set():
                    raise OperationCancelled('Operação cancelada')
                zf.writestr(entry['path'], entry['content'].encode('utf-8'))
                progress_cb(index + 1, len(entries))
...
class CodeCopierApp(_BaseTk):
    TAB_IDS = ['ext', 'files', 'search', 'explorer', 'arb', 'gi', 'restore']
    TAB_LABELS = ['Extensões', 'Arquivos', 'Buscar Nome', 'Explorador', 'Avulsos', 'Gitignore', 'Restaurar']
    ...
    def __init__(self):
        ...
        self.restore_cancel_event = threading.Event()
        self.src_root = ''
        self.src_dir_label = ''
        self.source_path = ''
        ...
        self.restore_warning_count = 0
        self.restore_total_chars = 0
        self.restore_txt_path = ''
        self.restore_generation = 0

        self._tree_path_resolvers = {}
        self._list_path_resolvers = {}
    ...
    def apply_theme(self):
        ...
        if getattr(self, 'restore_drop_zone', None) is not None:
            self.restore_drop_zone.configure(bg=surface2, fg=text2)
        if hasattr(self, 'restore_warnings_text'):
            self.restore_warnings_text.configure(
                bg=surface2, fg=text, insertbackground=text, selectbackground=select_bg,
    ...
    def _build_header(self, parent):
        ...
        ttk.Label(self.header_frame, text="📁 Copiador de Código — Python Edition v3.4",
    ...
    def _build_config(self, parent):
        ...
        self.src_input_entry = ttk.Entry(row1, textvariable=self.src_input_var)
        self.src_input_entry.pack(side='left', fill='x', expand=True, padx=6)
        self.src_input_entry.bind('<Return>', self.load_source_from_entry)
        ...
        self.btn_reload = ttk.Button(row1, text="↻", width=3, command=self.reload_source)
        self.btn_reload.pack(side='left', padx=(2, 0))
    ...
    def _build_tabs(self, parent):
        ...
        self._build_tab_restore(self.tab_frames['restore'])
    ...
    def _on_tab_changed(self, event):
        ...
        if not hasattr(self, 'counter_frame') or not hasattr(self, 'actions_frame'):
            return
        if self.active_tab == 'restore':
            ...
            if not self.actions_frame.winfo_manager():
                self.actions_frame.pack(fill='x', before=self.log_frame)
            if not self.counter_frame.winfo_manager():
                anchor = (self.progress_wrap if self.progress_wrap.winfo_manager()
                          else self.actions_frame)
                self.counter_frame.pack(fill='x', before=anchor)
    ...
    def _build_tab_search(self, parent):
        ...
        self.txt_search = tk.Text(parent, height=2, undo=True, autoseparators=True)
    ...
    # ABA: RESTAURAR
    # ---------------------------------------------------------
    def _build_tab_restore(self, parent):
        ttk.Label(
            parent,
            text="Abra um codigo_completo.txt para visualizar e reconstruir o projeto.",
            foreground='#64748b',
        ).pack(anchor='w', pady=(0, 4))

        if DND_AVAILABLE:
            self.restore_drop_zone = tk.Label(
                parent, text="⬇  Arraste o TXT do CodeFileCopier aqui  ⬇",
                relief='ridge', bd=1, padx=8, pady=6, foreground='#64748b')
            self.restore_drop_zone.pack(fill='x', pady=(0, 5))
        else:
            self.restore_drop_zone = None

        source_row = ttk.Frame(parent)
        source_row.pack(fill='x', pady=(0, 5))
        ttk.Label(source_row, text="TXT:").pack(side='left')
        self.restore_path_var = tk.StringVar()
        self.restore_path_entry = ttk.Entry(
            source_row, textvariable=self.restore_path_var, state='readonly')
        self.restore_path_entry.pack(side='left', fill='x', expand=True, padx=6)
        ttk.Button(source_row, text="TXT…", command=self.pick_restore_txt).pack(side='left', padx=2)
        ttk.Button(source_row, text="Limpar", command=self.clear_restore).pack(side='left', padx=(2, 0))

        summary = ttk.Frame(parent)
        summary.pack(fill='x', pady=(0, 5))
        self.restore_summary_lbl = ttk.Label(
            summary, text="Nenhum TXT carregado.", foreground='#64748b')
        self.restore_summary_lbl.pack(side='left')
        self.restore_warning_lbl = ttk.Label(summary, text="", foreground='#d97706')
        self.restore_warning_lbl.pack(side='right')

        panes = ttk.Panedwindow(parent, orient='horizontal')
        panes.pack(fill='both', expand=True)
        files_frame = ttk.LabelFrame(panes, text="Arquivos encontrados", padding=4)
        warnings_frame = ttk.LabelFrame(panes, text="Avisos", padding=4)
        panes.add(files_frame, weight=3)
        panes.add(warnings_frame, weight=2)

        self.restore_tree = ttk.Treeview(
            files_frame, columns=('path', 'chars'), show='headings', selectmode='browse')
        self.restore_tree.heading('path', text='Caminho restaurado')
        self.restore_tree.heading('chars', text='Caracteres')
        self.restore_tree.column('path', width=500, anchor='w')
        self.restore_tree.column('chars', width=100, anchor='e', stretch=False)
        restore_scroll = ttk.Scrollbar(files_frame, orient='vertical', command=self.restore_tree.yview)
        self.restore_tree.configure(yscrollcommand=restore_scroll.set)
        self.restore_tree.pack(side='left', fill='both', expand=True)
        restore_scroll.pack(side='right', fill='y')

        self.restore_warnings_text = tk.Text(
            warnings_frame, wrap='word', height=6, state='disabled')
        warning_scroll = ttk.Scrollbar(
            warnings_frame, orient='vertical', command=self.restore_warnings_text.yview)
        self.restore_warnings_text.configure(yscrollcommand=warning_scroll.set)
        self.restore_warnings_text.pack(side='left', fill='both', expand=True)
        warning_scroll.pack(side='right', fill='y')

        actions = ttk.Frame(parent)
        actions.pack(fill='x', pady=(5, 0))
        self.btn_restore_folder = ttk.Button(
            actions, text="📂 Restaurar em pasta…", command=self.restore_to_folder,
            state='disabled')
        self.btn_restore_folder.pack(side='left', padx=(0, 4))
        self.btn_restore_zip = ttk.Button(
            actions, text="⬇ Salvar projeto em ZIP…", command=self.save_restored_zip,
            state='disabled')
        self.btn_restore_zip.pack(side='left', padx=(0, 4))
        self.btn_restore_cancel = ttk.Button(
            actions, text="Cancelar", command=self.cancel_current_operation,
            state='disabled')
        self.btn_restore_cancel.pack(side='right')

    def pick_restore_txt(self):
        path = filedialog.askopenfilename(
            title="Selecione o TXT criado pelo CodeFileCopier",
            filetypes=[('Arquivo de texto', '*.txt'), ('Todos os arquivos', '*.*')])
        if path:
            self._start_restore_parse(path)

    def _start_restore_parse(self, path):
        path = os.path.abspath(os.path.expanduser(os.path.expandvars(str(path).strip(' "'))))
        if not os.path.isfile(path):
            self.toast('O TXT selecionado não existe.', 'err')
            return
        try:
            if os.path.getsize(path) > RESTORE_MAX_INPUT_BYTES:
                self.toast('O TXT ultrapassa o limite de segurança.', 'err')
                return
        except OSError as exc:
            self.toast(f'Não foi possível ler o TXT: {exc}', 'err')
            return

        self.restore_cancel_event.set()
        self.restore_cancel_event = threading.Event()
        self.restore_generation += 1
        token = self.restore_generation
        cancel_event = self.restore_cancel_event
        self.restore_entries = []
        self.restore_warnings = []
        self.restore_warning_count = 0
        self.restore_total_chars = 0
        self.restore_txt_path = path
        self.restore_path_var.set(path)
        self._render_restore_preview()
        self.btn_restore_cancel.config(state='normal')
        self.set_progress('pulse', 100, 'Lendo o TXT em segundo plano…')
        self.set_status('Analisando TXT…')
        self.log(f'⟳ Analisando TXT para restauração: {path}', 'info')

        def worker():
            try:
                def progress_cb(phase, current=0, total=0):
                    self.task_queue.put(
                        ('restore_parse_progress', token, phase, current, total))
                result = parse_codefilecopier_txt(
                    path, progress_cb=progress_cb, cancel_event=cancel_event)
                self.task_queue.put(('restore_parse_done', token, path, result))
            except OperationCancelled:
                self.task_queue.put(('restore_cancelled', token, 'Leitura do TXT cancelada.'))
            except Exception as exc:
                self.task_queue.put(
                    ('restore_error', token, f'Não foi possível interpretar o TXT: {exc}',
                     traceback.format_exc()))

        threading.Thread(target=worker, daemon=True).start()

    def _on_restore_parse_done(self, path, result):
        self.restore_txt_path = path
        self.restore_entries = sort_list_by(
            result['entries'], lambda entry: entry['path'], self.sort_var.get() == 'Natural')
        self.restore_warnings = result['warnings']
        self.restore_warning_count = result['warning_count']
        self.restore_total_chars = result['total_chars']
        self._render_restore_preview()
        self.set_progress(None)
        self.btn_restore_cancel.config(state='disabled')
        self.set_status(f"✓ {len(self.restore_entries)} arquivo(s) prontos para restaurar")
        self.log(
            f"✓ TXT analisado: {len(self.restore_entries)} arquivo(s), "
            f"{self.restore_warning_count} aviso(s).", 'ok')
        self.toast('TXT analisado com sucesso!',
                   'warn' if self.restore_warning_count else 'ok')

    def _render_restore_preview(self):
        if not hasattr(self, 'restore_tree'):
            return
        self.restore_tree.delete(*self.restore_tree.get_children())
        preview = self.restore_entries[:RESTORE_PREVIEW_LIMIT]
        for index, entry in enumerate(preview):
            self.restore_tree.insert(
                '', 'end', iid=f'restore::{index}',
                values=(entry['path'], f"{entry['chars']:,}".replace(',', '.')))
        count = len(self.restore_entries)
        if count:
            preview_note = (f' — prévia limitada a {RESTORE_PREVIEW_LIMIT:,}'.replace(',', '.')
                            if count > RESTORE_PREVIEW_LIMIT else '')
            self.restore_summary_lbl.config(
                text=f"{count:,} arquivo(s) | {self.restore_total_chars:,} caracteres{preview_note}"
                     .replace(',', '.'))
        else:
            self.restore_summary_lbl.config(text='Nenhum TXT carregado.')
        warning_text = f"{self.restore_warning_count} aviso(s)" if self.restore_warning_count else ''
        self.restore_warning_lbl.config(text=warning_text)
        self.restore_warnings_text.configure(state='normal')
        self.restore_warnings_text.delete('1.0', 'end')
        if self.restore_warnings:
            self.restore_warnings_text.insert('1.0', '\n'.join('• ' + w for w in self.restore_warnings))
            if self.restore_warning_count > len(self.restore_warnings):
                self.restore_warnings_text.insert(
                    'end', f"\n• … e mais {self.restore_warning_count - len(self.restore_warnings)} aviso(s).")
        else:
            self.restore_warnings_text.insert('1.0', 'Nenhum aviso.')
        self.restore_warnings_text.configure(state='disabled')
        state = 'normal' if count else 'disabled'
        self.btn_restore_folder.config(state=state)
        self.btn_restore_zip.config(state=state)
    ...
    def clear_restore(self):
        ...
        self.restore_total_chars = 0
        self.restore_txt_path = ''
        if hasattr(self, 'restore_path_var'):
            self.restore_path_var.set('')
            self._render_restore_preview()
            self.btn_restore_cancel.config(state='disabled')
        self.set_progress(None)

    def restore_to_folder(self):
        if not self.restore_entries:
            self.toast('Carregue um TXT primeiro.', 'warn')
            return
        destination = filedialog.askdirectory(title="Selecione a pasta de destino")
        if not destination:
            return
        self._start_restore_output('folder', destination)

    def save_restored_zip(self):
        if not self.restore_entries:
            self.toast('Carregue um TXT primeiro.', 'warn')
            return
        source_name = os.path.splitext(os.path.basename(self.restore_txt_path))[0]
        source_name = re.sub(r'_codigo_completo$', '', source_name, flags=re.IGNORECASE)
        if not source_name or source_name.lower() == 'codigo_completo':
            source_name = 'projeto'
        path = filedialog.asksaveasfilename(
            title="Salvar projeto restaurado em ZIP",
            initialfile=source_name + '_restaurado.zip',
            defaultextension='.zip',
            filetypes=[('Arquivo ZIP', '*.zip'), ('Todos os arquivos', '*.*')])
        if path:
            self._start_restore_output('zip', path)

    def _start_restore_output(self, mode, destination):
        self.restore_cancel_event.set()
        self.restore_cancel_event = threading.Event()
        self.restore_generation += 1
        token = self.restore_generation
        cancel_event = self.restore_cancel_event
        entries = list(self.restore_entries)
        self.btn_restore_folder.config(state='disabled')
        self.btn_restore_zip.config(state='disabled')
        self.btn_restore_cancel.config(state='normal')
        label = 'Restaurando arquivos…' if mode == 'folder' else 'Criando ZIP restaurado…'
        self.set_progress(0, len(entries), label)
        self.set_status(label)

        def progress_cb(current, total):
            self.task_queue.put(('restore_output_progress', token, current, total, mode))

        def worker():
            try:
                if mode == 'folder':
                    restored, warnings = restore_entries_to_folder(
                        entries, destination, progress_cb, cancel_event)
                    result = {'restored': restored, 'warnings': warnings, 'destination': destination}
                else:
                    zip_path = restore_entries_to_zip(
                        entries, destination, progress_cb, cancel_event)
                    result = {'restored': len(entries), 'warnings': [], 'destination': zip_path}
                self.task_queue.put(('restore_output_done', token, mode, result))
            except OperationCancelled:
                self.task_queue.put(('restore_cancelled', token, 'Restauração cancelada.'))
            except Exception as exc:
                self.task_queue.put(
                    ('restore_error', token, f'Falha durante a restauração: {exc}',
                     traceback.format_exc()))
    ...
    def cancel_current_operation(self):
        ...
        self.log('Cancelamento solicitado pelo usuário.', 'warn')
    ...
    def _build_statusbar(self, parent):
        ...
        self.st_right_lbl = ttk.Label(self.statusbar_frame, text="v3.4 — Python Edition", foreground='#64748b')
    ...
    def set_progress(self, val, maximum=100, label=''):
        ...
        if not self.progress_wrap.winfo_ismapped():
            anchor = (self.actions_frame if self.actions_frame.winfo_manager()
                      else self.log_frame)
            self.progress_wrap.pack(fill='x', pady=(0, 2), before=anchor)
        if val == 'pulse':
    ...
    def refresh_all_lists(self):
        ...
        if self.active_tab == 'explorer':
            self.populate_explorer()
        if self.restore_entries:
            self.restore_entries = sort_list_by(
                self.restore_entries, lambda entry: entry['path'],
                self.sort_var.get() == 'Natural')
            self._render_restore_preview()
        self.update_counter()

    # ---------------------------------------------------------
    # MENUS DE CONTEXTO: TEXTO E CAMINHOS
    # ---------------------------------------------------------
    def _setup_context_menus(self):
        self.text_context_menu = tk.Menu(self, tearoff=False)
        self.path_context_menu = tk.Menu(self, tearoff=False)
        for widget_class in ('TEntry', 'Entry', 'Text', 'TCombobox'):
            self.bind_class(widget_class, '<Button-3>', self._show_text_context, add='+')
            self.bind_class(widget_class, '<Control-Button-1>', self._show_text_context, add='+')
            self.bind_class(widget_class, '<Control-a>', self._context_select_all, add='+')
            self.bind_class(widget_class, '<Control-A>', self._context_select_all, add='+')

        self._bind_tree_path_menu(self.file_tree, self._resolve_file_tree_path)
        self._bind_tree_path_menu(self.search_tree, self._resolve_search_tree_path)
        self._bind_tree_path_menu(self.explorer_tree, self._resolve_explorer_path)
        self._bind_tree_path_menu(self.arb_tree, self._resolve_arb_path)
        self._bind_tree_path_menu(self.restore_tree, self._resolve_restore_path)
        self._bind_list_path_menu(self.gi_preview, self._resolve_gi_preview_path)

    def _text_is_editable(self, widget):
        try:
            return str(widget.cget('state')) not in ('disabled', 'readonly')
        except (tk.TclError, AttributeError):
            return True

    def _text_has_selection(self, widget):
        try:
    ...
    def _show_text_context(self, event):
        ...
        menu.delete(0, 'end')
        editable = self._text_is_editable(widget)
        selected = self._text_has_selection(widget)
        paste_available = editable and self._clipboard_has_text()
        menu.add_command(label='Desfazer', command=lambda: self._context_edit('undo'),
                         state='normal' if editable else 'disabled')
        menu.add_command(label='Refazer', command=lambda: self._context_edit('redo'),
                         state='normal' if editable else 'disabled')
        menu.add_separator()
        menu.add_command(label='Recortar', command=lambda: self._context_edit('cut'),
                         state='normal' if editable and selected else 'disabled')
        menu.add_command(label='Copiar', command=lambda: self._context_edit('copy'),
                         state='normal' if selected else 'disabled')
        menu.add_command(label='Colar', command=lambda: self._context_edit('paste'),
                         state='normal' if paste_available else 'disabled')
        menu.add_command(label='Excluir', command=lambda: self._context_edit('delete'),
                         state='normal' if editable and selected else 'disabled')
        menu.add_separator()
        menu.add_command(label='Selecionar tudo', command=lambda: self._context_edit('select_all'))
        try:
    ...
    def _delete_selected_text(self, widget):
        if not self._text_has_selection(widget):
            return
        if isinstance(widget, tk.Text):
    ...
    def _context_select_all(self, event):
        self._context_widget = event.widget
        self._context_edit('select_all')
        return 'break'

    def _bind_tree_path_menu(self, tree, resolver):
        self._tree_path_resolvers[tree] = resolver
        tree.bind('<Button-3>', self._show_tree_path_context, add='+')
        tree.bind('<Control-Button-1>', self._show_tree_path_context, add='+')

    def _bind_list_path_menu(self, listbox, resolver):
        self._list_path_resolvers[listbox] = resolver
        listbox.bind('<Button-3>', self._show_list_path_context, add='+')
        listbox.bind('<Control-Button-1>', self._show_list_path_context, add='+')

    def _show_tree_path_context(self, event):
        tree = event.widget
        iid = tree.identify_row(event.y)
        if not iid or iid == 'empty':
            return 'break'
        ...
        tree.focus(iid)
        resolver = self._tree_path_resolvers.get(tree)
        paths = resolver(iid) if resolver else None
        self._popup_path_menu(paths, event.x_root, event.y_root)
        return 'break'

    def _show_list_path_context(self, event):
        ...
        resolver = self._list_path_resolvers.get(listbox)
        paths = resolver(index) if resolver else None
        self._popup_path_menu(paths, event.x_root, event.y_root)
        return 'break'

    def _popup_path_menu(self, paths, x_root, y_root):
        if not paths:
            return
        relative_path, full_path = paths
        menu = self.path_context_menu
        menu.delete(0, 'end')
        menu.add_command(
            label='Copiar caminho completo',
            command=lambda value=full_path: self._copy_path(value),
            state='normal' if full_path else 'disabled')
        menu.add_command(
            label='Copiar caminho relativo',
            command=lambda value=relative_path: self._copy_path(value),
            state='normal' if relative_path else 'disabled')
        try:
            menu.tk_popup(x_root, y_root)
        finally:
            menu.grab_release()

    def _copy_path(self, value):
        if not value:
            return
        self.clipboard_clear()
        self.clipboard_append(str(value))
        self.update_idletasks()
        self.toast('Caminho copiado!', 'ok')

    def _paths_for_rel(self, rel_path):
        meta = next((item for item in self.all_meta if item['rel_path'] == rel_path), None)
        if meta:
            if meta.get('abs_path'):
                return rel_path, os.path.abspath(meta['abs_path'])
            if meta.get('zip_path'):
                return rel_path, f"{os.path.abspath(meta['zip_path'])} :: {meta['zip_member']}"
        if self.source_path:
            if self.source_is_zip:
                return rel_path, f"{self.source_path} :: {rel_path}"
            return rel_path, os.path.abspath(
                os.path.join(os.path.dirname(self.source_path), *rel_path.split('/')))
        return rel_path, rel_path

    def _resolve_file_tree_path(self, iid):
        return self._paths_for_rel(iid.split('::', 1)[1])

    def _resolve_search_tree_path(self, iid):
        return self._paths_for_rel(iid.split('::', 1)[1])

    def _resolve_explorer_path(self, iid):
        if iid.startswith('F::'):
            return self._paths_for_rel(iid[3:])
        if iid.startswith('D::'):
            rel_path = iid[3:].rstrip('/')
            if self.source_path and self.source_is_zip:
                return rel_path, f"{self.source_path} :: {rel_path}/"
            if self.source_path:
    ...
    def _resolve_arb_path(self, iid):
        index = int(iid.split('::', 1)[1])
        if not 0 <= index < len(self.arb_files):
            return None
        item = self.arb_files[index]
        return item['name'], os.path.abspath(item['abs_path'])

    def _resolve_restore_path(self, iid):
        index = int(iid.split('::', 1)[1])
        if not 0 <= index < min(len(self.restore_entries), RESTORE_PREVIEW_LIMIT):
            return None
        rel_path = self.restore_entries[index]['path']
        full_path = f"{self.restore_txt_path} :: {rel_path}" if self.restore_txt_path else rel_path
        return rel_path, full_path

    def _resolve_gi_preview_path(self, index):
        if not 0 <= index < min(len(self.all_meta), 400):
            return None
        return self._paths_for_rel(self.all_meta[index]['rel_path'])
    ...
    def _setup_dnd(self):
        ...
        try:
            ...
            # TXT de restauração
            for w in (self.restore_drop_zone, self.restore_path_entry, self.restore_tree):
                if w is None:
                    continue
                w.drop_target_register(DND_FILES)
                w.dnd_bind('<<Drop>>', self._on_restore_drop)
                w.dnd_bind('<<DropEnter>>', lambda e: self._on_dnd_hover(self.restore_drop_zone, True))
                w.dnd_bind('<<DropLeave>>', lambda e: self._on_dnd_hover(self.restore_drop_zone, False))

            self.log("Arrastar-e-soltar habilitado para pasta, ZIP, avulsos e TXT de restauração.", 'ok')
        except Exception as e:
    ...
    def _on_restore_drop(self, event):
        self._on_dnd_hover(self.restore_drop_zone, False)
        paths = self._parse_dnd_paths(event.data)
        if not paths:
            return
        path = paths[0]
        if os.path.isfile(path) and path.lower().endswith('.txt'):
            ...
            self.toast('Arraste um arquivo TXT criado pelo CodeFileCopier.', 'warn')
    ...
    def pick_source_dir(self):
        ...
        if path:
            self._start_scan(path, is_zip=False, preserve_selection=False)
    ...
    def pick_zip(self):
        path = filedialog.askopenfilename(
            ...
            filetypes=[('Arquivo ZIP', '*.zip'), ('Todos os arquivos', '*.*')])
        if path:
            self._start_scan(path, is_zip=True, preserve_selection=False)

    def _normalized_source_path(self, raw_path):
        path = str(raw_path or '').strip().strip('"')
        path = os.path.expanduser(os.path.expandvars(path))
        return os.path.abspath(path) if path else ''

    def load_source_from_entry(self, event=None):
        path = self._normalized_source_path(self.src_input_var.get())
        if not path:
            self.toast('Digite ou cole o caminho de uma pasta ou ZIP.', 'warn')
        ...
        if os.path.isdir(path):
            ...
            self.toast('O caminho deve apontar para uma pasta ou arquivo ZIP existente.', 'err')
            return 'break' if event else None
        same_source = (os.path.normcase(path) == os.path.normcase(self.source_path) and
                       is_zip == self.source_is_zip)
        self._start_scan(path, is_zip=is_zip, preserve_selection=same_source)
        return 'break' if event else None

    def reload_source(self):
        """Relê a origem atual ou carrega o caminho digitado no campo Entrada."""
        self.load_source_from_entry()

    def _start_scan(self, path, is_zip, preserve_selection=None):
        path = self._normalized_source_path(path)
        if (is_zip and not os.path.isfile(path)) or (not is_zip and not os.path.isdir(path)):
            self.toast('A origem não existe mais ou não está acessível.', 'err')
            return
        if preserve_selection is None:
            preserve_selection = (
                os.path.normcase(path) == os.path.normcase(self.source_path) and
                bool(is_zip) == self.source_is_zip)

        self.cancel_event.set()
        self.cancel_event = threading.Event()
        self.scan_generation += 1
        token = self.scan_generation
        self.src_input_var.set(path)
        ...
        self.set_status('Recarregando origem…' if preserve_selection else 'Lendo metadados…')
        self.log(
            ('↻ Recarregando: ' if preserve_selection else '⟳ Iniciando varredura: ') + path,
            'info')

        # Variáveis do Tkinter são resolvidas na thread principal.
        ...
        def worker():
            try:
                def progress_cb(n):
                    self.task_queue.put(('scan_progress', token, n))

                if is_zip:
                    metas, root_name = scan_zip(
                        path, ignore_patterns, size_limit_kb, progress_cb, cancel_event)
                else:
                    metas, root_name = scan_directory(
                        path, ignore_patterns, size_limit_kb, progress_cb, cancel_event)
                if cancel_event.is_set():
                    return
                self.task_queue.put(
                    ('scan_done', token, metas, root_name, path, bool(is_zip),
                     bool(preserve_selection)))
            except Exception as exc:
                self.task_queue.put(
                    ('scan_error', token, str(exc), traceback.format_exc()))
        ...
        try:
            while True:
                ...
                if kind == 'scan_progress':
                    token, n = item[1], item[2]
                    if token == self.scan_generation:
                        self.set_progress(
                            'pulse', 100, f'Analisando… {n} arquivo(s) encontrados')
                elif kind == 'scan_done':
                    token = item[1]
                    if token == self.scan_generation:
                        self._on_scan_done(
                    ...
                    if token == self.scan_generation:
                        ...
                        if self.source_path:
                            self.src_input_var.set(self.source_path)
                        self.toast('Não foi possível ler a pasta/ZIP.', 'err')
                        self.log(f"[ERRO] {item[2]}", 'err')
                ...
                elif kind == 'restore_parse_progress':
                    token, phase, current, total = item[1:5]
                    if token == self.restore_generation:
                        if phase == 'reading':
                            if total:
                                self.set_progress(
                                    current, total,
                                    f'Lendo TXT… {fmt_size(current)}/{fmt_size(total)}')
                            else:
                                self.set_progress('pulse', 100, 'Lendo o TXT em segundo plano…')
                        elif total:
                            self.set_progress(
                                current, total,
                                f'Analisando cabeçalhos… {current}/{total}')
                        else:
                            self.set_progress('pulse', 100, 'Analisando cabeçalhos…')
                elif kind == 'restore_parse_done':
                    token = item[1]
                    if token == self.restore_generation:
                        self._on_restore_parse_done(item[2], item[3])
                elif kind == 'restore_output_progress':
                    token, current, total, mode = item[1:5]
                    if token == self.restore_generation:
                        label = ('Restaurando arquivos…' if mode == 'folder'
                                 else 'Criando ZIP restaurado…')
                        self.set_progress(current, total, f'{label} {current}/{total}')
                elif kind == 'restore_output_done':
                    token, mode, result = item[1], item[2], item[3]
                    if token == self.restore_generation:
                        self.set_progress(None)
                        self.btn_restore_cancel.config(state='disabled')
                        self.btn_restore_folder.config(state='normal')
                        self.btn_restore_zip.config(state='normal')
                        warning_count = len(result['warnings'])
                        destination = result['destination']
                        self.set_status(
                            f"✓ {result['restored']} arquivo(s) restaurados")
                        self.log(
                            f"✓ Restauração concluída em {destination}: "
                            f"{result['restored']} arquivo(s), {warning_count} falha(s).",
                            'warn' if warning_count else 'ok')
                        for warning in result['warnings'][:30]:
                            self.log('[AVISO] ' + warning, 'warn')
                        self.toast(
                    ...
                    if token == self.restore_generation:
                        self.set_progress(None)
                        self.btn_restore_cancel.config(state='disabled')
                        state = 'normal' if self.restore_entries else 'disabled'
                        self.btn_restore_folder.config(state=state)
                        self.btn_restore_zip.config(state=state)
                        self.set_status('Cancelado')
                        self.log(item[2], 'warn')
                        self.toast(item[2], 'warn')
                    ...
                    if token == self.restore_generation:
                        ...
                        self.btn_restore_folder.config(state=state)
                        self.btn_restore_zip.config(state=state)
                        self.set_status('Erro na restauração')
                        self.log('[ERRO] ' + item[2], 'err')
                        self.toast(item[2], 'err')
        except queue.Empty:
            pass
        self.after(60, self._poll_queue)

    def _on_scan_done(self, metas, root_name, source_path, is_zip,
                      preserve_selection):
        old_meta = {item['rel_path']: item for item in self.all_meta}
        old_selected_files = set(self.selected_files)
        old_selected_exts = set(self.selected_exts)
        self.all_meta = sort_list_by(
            metas, lambda m: m['rel_path'], self.sort_var.get() == 'Natural')
        self.source_path = source_path
        self.source_is_zip = bool(is_zip)
        self.src_input_var.set(source_path)
        self.src_root = root_name

        ext_set = sorted(set(m['ext'] for m in self.all_meta))
        self.all_exts = ext_set
        new_paths = {item['rel_path'] for item in self.all_meta}
        if preserve_selection:
            self.selected_files = old_selected_files & new_paths
            self.selected_exts = old_selected_exts & set(self.all_exts)
            self.search_results = [path for path in self.search_results if path in new_paths]
        else:
        ...
        if preserve_selection:
            added = len(new_paths - set(old_meta))
            removed = len(set(old_meta) - new_paths)
            modified = sum(
                1 for item in self.all_meta
                if item['rel_path'] in old_meta and any((
                    item.get('size') != old_meta[item['rel_path']].get('size'),
                    item.get('mtime_ns') != old_meta[item['rel_path']].get('mtime_ns'),
                    item.get('crc') != old_meta[item['rel_path']].get('crc'),
                )))
            self.log(
                f"✓ Origem recarregada: {len(self.all_meta)} arquivo(s); "
                f"{added} novo(s), {modified} alterado(s), {removed} removido(s).",
                'ok')
        else:
            self.log(
                f"✓ {len(self.all_meta)} arquivo(s), "
                f"{len(self.all_exts)} extensão(ões).", 'ok')
        self.update_counter()
    ...
    def start_copy(self):
        ...
        self.gen_output(metas, src, self.src_root or os.path.basename(src), out_name)

    def gen_output(self, metas, source_display, tree_root, out_name):
        ...
        def worker():
            try:
                ...
                out = build_header(source_display, len(metas), filters)
                copied = 0
                skipped = 0
                ...
                out += '\n' + '=' * 42 + '\nEstrutura de pastas:\n' + '=' * 42 + '\n'
                out += build_tree_txt(metas, tree_root)
    ...
    def clear_all(self):
        ...
        self.update_counter()
        self.set_status('Pronto', 'v3.4 — Python Edition')
